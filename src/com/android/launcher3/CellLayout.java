...
public class CellLayout extends ViewGroup {
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		//appListView.setFocusableInTouchMode(false);
		//appIconView.setOnClickListener(this);
		
		appListView.setPadding(0, 50, 0, 50);
        addView(appListView);
		addView(appIconView);
		//CG wujiangwei Add E for Launcher for Glass 2015.09.30
        }
    ...
    class MyAdapter extends BaseAdapter implements OnItemSelectedListener,OnScrollListener, OnItemClickListener{
        ...
        public ResolveInfo getItem(int position) {
            return resolveInfos.get(position % resolveInfos.size());
        }
        @Override
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if(convertView == null){
                ...
                //convertView = getLayoutInflater().inflate(R.layout.appicon_list_item, null);
                vh.iv = (ImageView) convertView.findViewById(R.id.appicon);
                //vh.tv = (TextView) convertView.findViewById(R.id.apppackage);
			...
			//vh.tv.setText(pkgName);
			// remove image drawable
			if(icon != null){
			    vh.iv.setImageDrawable(icon);
			}else{
			   Log.i("wujiangwei", "icon is null");
			}
		...
		public void onScrollStateChanged(final AbsListView view, int scrollState) {
		    Log.i("wujiangwei","onScrollStateChanged,scrollState is " + scrollState);
			
			switch (scrollState) {
			case AbsListView.OnScrollListener.SCROLL_STATE_FLING:
				Log.i("wujiangwei","onScrollStateChanged,scrollState is " + SCROLL_STATE_FLING);
				break;
			case AbsListView.OnScrollListener.SCROLL_STATE_IDLE:
				Log.i("wujiangwei","onScrollStateChanged,scrollState is " + SCROLL_STATE_IDLE);
				int firstPostion = view.getFirstVisiblePosition();
				View firstChild = view.getChildAt(0);
				final int height = firstChild.getHeight();
				final int top = -(firstChild.getTop());
				if (top > (height/2)) {
					Log.i("wujiangwei","onScrollStateChanged,firstPostion = " + firstPostion);
					// view.smoothScrollToPosition(firstPostion + 1);
					view.post(new Runnable() {

						@Override
						public void run() {
								view.smoothScrollBy(height-top-50, 200);
							}
					});
					...
					view.post(new Runnable() {
						...
						@Override
						public void run() {
							view.smoothScrollBy(-top-50, 200);
						}
					});
				}
				Log.i("wujiangwei","onScrollStateChanged,top = " + top);
				Log.i("wujiangwei","onScrollStateChanged,height = " + height);
			break;
			case AbsListView.OnScrollListener.SCROLL_STATE_TOUCH_SCROLL:
				Log.i("wujiangwei","onScrollStateChanged,scrollState is " + SCROLL_STATE_TOUCH_SCROLL);
				break;
			default:
				break;
			}
		...
		public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
			...
			if (firstVisibleItem <= 2) {
			    appListView.setSelection(resolveInfos.size() + 2);
		    } else if (firstVisibleItem + visibleItemCount > getCount() - 2) {
			    appListView.setSelection(firstVisibleItem - resolveInfos.size());
		    }
			//appIconView.setImageDrawable(getItem(firstVisibleItem).loadIcon(pm));
		    ScaleType scaleType = ScaleType.CENTER;//FIT_START;//FIT_XY;//////FIT_CENTER;
		    appIconView.setScaleType(scaleType );
			//appIconView.setLeft(40);
			//appIconView.offsetLeftAndRight(10);
			
			//LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
            //lp.setMargins(0, 0, 100, 0);
            //appIconView.setLayoutParams(lp);
			//appIconView.setPadding(8, 8, 8, 8); 
			appIconView.setImageDrawable(getItem(getFirstVisibleItem()).loadIcon(pm));
		}
	...
	public void  startSelectedApp() {	
		String pkgName1 = ((MyAdapter) appListView.getAdapter()).getFirstVisiblePkgName();
		String activityName1 = ((MyAdapter) appListView.getAdapter()).getFirstVisibleActivityName();
